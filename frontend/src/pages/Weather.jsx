...
const WeatherPage = () => {
  ...
  if (!weather && !hasWeatherRef.current && weatherLoading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center h-full text-gray-500 dark:text-gray-400 w-full p-6">
  ...
  if (!weather && weatherError) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center h-full w-full p-6">
        <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-card flex flex-col items-center justify-center text-center">
          <span className="text-4xl mb-3">☁️</span>
          <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-2">{weatherError}</h2>
          <p className="text-gray-500 dark:text-gray-400 text-sm">Attempting to reload might fix this issue.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col p-5 sm:p-6 lg:p-8 space-y-6 overflow-y-auto max-w-[1400px] w-full mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-gray-100 tracking-tight">Weather Dashboard</h1>
          <div className="mt-2 flex flex-col">
            ...
            {hasValidLocation && locSecondaryLine && (
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium mt-0.5 pl-6">
                {locSecondaryLine}
              </p>
            )}
            {isFallback && (
        ...
        <div className="flex flex-col items-start md:items-end">
          ...
          {isCached ? (
            <div className="bg-gray-100 px-3 py-1.5 rounded-full flex items-center gap-2 border border-gray-200 dark:border-gray-700 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-[32px] p-8 shadow-card border border-gray-100 dark:border-gray-700 relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start justify-between gap-8">
              <div className="flex items-center gap-6">
                ...
                <div className="flex flex-col">
                  <span className="text-6xl sm:text-7xl font-bold text-gray-800 dark:text-gray-100 tracking-tighter">
                    {current?.temperature != null ? Math.round(current.temperature) : '--'}°C
                  </span>
                  <span className="text-xl font-medium text-gray-500 dark:text-gray-400 mt-1">{condition.label}</span>
                </div>
              </div>
              ...
              <div className="flex sm:flex-col gap-4 sm:gap-3 w-full sm:w-auto bg-gray-50/80 sm:bg-transparent p-4 sm:p-0 rounded-2xl sm:rounded-none">
                <div className="flex items-center gap-3 flex-1 sm:flex-none">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-lg font-bold text-gray-700 dark:text-gray-200">{current?.humidity != null ? `${Math.round(current.humidity)}%` : '--'}</span>
                  </div>
                </div>
                <div className="w-[1px] bg-gray-200 sm:h-[1px] sm:w-full"></div>
                <div className="flex items-center gap-3 flex-1 sm:flex-none">
                  <div className="w-10 h-10 rounded-xl bg-gray-100 text-gray-500 dark:text-gray-400 flex items-center justify-center text-lg">💨</div>
                  <div className="flex flex-col">
                    <span className="text-xs text-gray-400 font-semibold uppercase">Wind</span>
                    <span className="text-lg font-bold text-gray-700 dark:text-gray-200">{current?.windSpeed != null ? `${Math.round(current.windSpeed)} km/h` : '--'}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 sm:p-8 shadow-card border border-gray-100 dark:border-gray-700">
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 snap-x scrollbar-hide -mx-2 px-2">
              {filteredDaily.map((day, idx) => {
                ...
                return (
                  <div key={day.date || idx} className={`snap-start min-w-[110px] flex-shrink-0 flex flex-col items-center justify-between p-4 rounded-2xl ${isToday ? 'bg-brand-surface border-2 border-brand-green/20' : 'bg-gray-50/80 border border-gray-100 dark:border-gray-700'} transition-all`}>
                    <span className="text-sm font-bold text-gray-500 dark:text-gray-400">{isToday ? 'TODAY' : dayName}</span>
                    <span className="text-4xl my-3 drop-shadow-sm">{dayCond.emoji}</span>
                    <div className="flex items-center gap-2 mb-3">
                      <span className="text-lg font-bold text-gray-800 dark:text-gray-100">{day.temperatureMax != null ? Math.round(day.temperatureMax) : '--'}°</span>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-card border border-gray-100 dark:border-gray-700">
            <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-5">Today's Summary</h2>
            
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100 dark:border-gray-700/50">
                <span className="text-xl mb-2 block">🌡️</span>
                <p className="text-[11px] text-gray-400 font-bold uppercase tracking-wide">High / Low</p>
                <p className="text-lg font-bold text-gray-800 dark:text-gray-100 mt-0.5">{todayMax}° / {todayMin}°</p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-card border border-gray-100 dark:border-gray-700">
            ...
            <ul className="space-y-4">
              <li className="flex gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-gray-700 dark:text-gray-200">Rain Chance</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Probability of rain today.</p>
                </div>
              </li>
              <li className="flex gap-3">
                <span className="text-lg shrink-0">🌧️</span>
                <div>
                  <p className="text-xs font-bold text-gray-700 dark:text-gray-200">Rainfall</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Expected rainfall amount.</p>
                </div>
              </li>
              <li className="flex gap-3">
                <span className="text-lg shrink-0">☀️</span>
                <div>
                  <p className="text-xs font-bold text-gray-700 dark:text-gray-200">ET₀</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Reference water loss from weather conditions.</p>
                </div>
              </li>
              <li className="flex gap-3">
                <span className="text-lg shrink-0">💨</span>
                <div>
                  <p className="text-xs font-bold text-gray-700 dark:text-gray-200">Wind</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Useful when planning spraying.</p>
                </div>
              </li>
            </ul>
          </div>
